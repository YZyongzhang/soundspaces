...
class Net(nn.Module):
    def __init__(self):
        ...
        # 明个进行三个测试实验
        # 1. 只用audio进行强化学习
        # 2. visual 和audio的掩码图片进行强化学习
        # 3. 最终结果只要能在测试集上有好的结果就算成功！
        # 4. 还可以尝试使用done进行训练一下？就是用step进行转化
    ...
    for epoch in range(num_epoch):
        for episode in range(num_episode):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                nums+=1
    # torch.save(cql.model.state_dict() , 'rl_model_visual.pth')

if __name__ == '__main__':
    from torch.utils.tensorboard import SummaryWriter
    log_dir = './logs/cql_visual_3'
    writer = SummaryWriter(log_dir)
    logging.basicConfig(filename='output_cql_visual_3.log', level=logging.INFO)
    train(writer)