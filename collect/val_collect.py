...
def collect():
    actor = Actor(config)
    seq_list = list()
    for num_episodes in range(319,600):
        t_start = time.time()
        result_lists ,_ ,_ = actor.rollout()
        path = os.path.join(f"data/RL/valdata/",  f"rl_episode_{num_episodes}.pkl")
        with open(path, "wb") as f:
            pickle.dump(result_lists, f)
        actor.path_point = list()
        seq_list.clear()
        current_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        logging.info(f" rl_episode_{num_episodes}.pkl")
        logging.info(f"episode time {(time.time() - t_start) // 60} m {(time.time() - t_start) % 60} s")
        logging.info(f"now time is {current_time_str}")
if  __name__== "__main__":
    logging.basicConfig(filename='./data/RL/valdata/RLDATA.log', level=logging.INFO)
    collect()
