...
def train():
    ...
    for epoch in range(numepoch):
        ...
        if episode % 2000 == 0:
            torch.save(model.state_dict() , f'../../data/checkpoint/acmcheckpoint/avf_{episode}.pth')
    torch.save(model.state_dict(), f'../../data/checkpoint/acmcheckpoint/avf_{episode}.pth')
    
    writer.close()
if __name__ == '__main__':
    from torch.utils.tensorboard import SummaryWriter
    log_dir = './logs/loss/avf'
    writer = SummaryWriter(log_dir)
    logging.basicConfig(filename='./logs/avf.log', level=logging.INFO)
    train()