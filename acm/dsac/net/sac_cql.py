"""
use the direact SAC-CQL and learn from a github project 
in some case , i think use dymic cql-aphla may have some error.
and a new idea in the sac_cql_1.py
"""
import torch
...
class DiscreteSAC_CQL:
    ...
    def train_step(self, pre_audio, pre_visual, next_audio, next_visual, labels, reward, done):
        ...
        # 求alphaloss
        # alpha_loss = - (self.log_alpha.exp() * (log_action_pi.cpu() + self.target_entropy).to(self.device).detach()).mean()
        ...
        q1_ = q1.gather(1, labels.unsqueeze(1).long())
        q2_ = q2.gather(1, labels.unsqueeze(1).long())
        
        critic1_loss = 0.5 * F.mse_loss(q1_, Q_target)
        critic2_loss = 0.5 * F.mse_loss(q2_, Q_target)
        ...
        return policy_loss.item(), current_alpha, critic1_loss.item(), critic2_loss.item(), cql1_scaled_loss.item(), cql2_scaled_loss.item(), current_alpha, cql_alpha_loss.item(), cql_alpha.item()
