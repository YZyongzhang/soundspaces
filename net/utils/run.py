...
def get_lmdb_database():
    # muti_env_data
    # T = trans_to_database_from_rawdata(store_path="muti_env_data")
...
def get_encode_database():
    from_path = f'{agent_config.RELATIVE_DATABASE_DIR}'+'muti_env'
    ...
    for levelnum , data_path in enumerate(data_paths):
        T.gen_database(database_path=data_path,want_gen_database='muti_env_encode',level=f'level{levelnum}')
    #################################################################
